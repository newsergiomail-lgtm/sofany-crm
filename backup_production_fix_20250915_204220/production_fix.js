...
router.get('/orders', authenticateToken, async (req, res) => {
  try {
    ...
    // Показываем точно те же заказы, что и в канбане
    let whereConditions = ['o.status IN (\'in_production\', \'new\')'];
    let queryParams = [];
    let paramCount = 0;
    ...
    const ordersQuery = `
      SELECT 
        ...
        po.updated_at as stage_updated_at
      FROM orders o
    ...
    if (orderResult.rows.length === 0) {
      ...
      if (existingQrResult.rows.length > 0) {
        ...
        return res.json({
          ...
          qr_code: {
            id: existingQr.id,
            current_stage_id: existingQr.current_stage_id,
