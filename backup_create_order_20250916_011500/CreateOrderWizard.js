...
const CreateOrderWizard = () => {
  ...
  const steps = [
    ...
    { id: 6, title: 'Готово', icon: Check, description: 'Заказ создан' }
  ];
  ...
  const createOrderMutation = useMutation(ordersAPI.create, {
    onSuccess: async (data) => {
      ...
      if (newId) {
        ...
        setCurrentStep(6); // Переходим на финальную страницу
  ...
  const createOrder = async () => {
    try {
      ...
      const result = await createOrderMutation.mutateAsync(apiBody);
      
      // Загружаем чертежи после создания заказа
      if (drawings.length > 0 && result?.order?.id) {
        await uploadDrawings(result.order.id);
      }
    } catch (error) {
      console.error('Ошибка создания заказа:', error);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-4">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {drawings.length > 0 && (
                      <div>
                        <span className="text-gray-600">Чертежи:</span>
                        <span className="ml-2 font-medium">{drawings.length} файл(ов)</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {currentStep === 6 && (
            <div>
              <div className="text-center mb-8">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Заказ успешно создан!</h2>
                <p className="text-gray-600">Ваш заказ принят в работу и передан в производство</p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {orderDetails.deadline ? new Date(orderDetails.deadline).toLocaleDateString() : 'Не указан'}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Чертежи:</span>
              ...
              {createdOrderId && (
                <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
                  <h3 className="text-lg font-semibold text-gray-900 mb-4">QR-код для отслеживания</h3>
                  <div className="flex flex-col items-center">
                    <QRCodeGenerator orderId={createdOrderId} />
                    <p className="text-sm text-gray-600 mt-3 text-center">
                      QR-код содержит ссылку для отслеживания статуса заказа в реальном времени
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-6">
                <h3 className="text-lg font-semibold text-blue-900 mb-4">Что происходит дальше?</h3>
                <div className="space-y-3">
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      <p className="font-medium text-blue-900">Заказ добавлен в заказ-наряд</p>
                      <p className="text-sm text-blue-700">Ваш заказ автоматически появится в системе производства</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0 w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                    ...
                    <div>
                      <p className="font-medium text-blue-900">Назначение ответственного</p>
                      <p className="text-sm text-blue-700">Менеджер назначит мастера и определит сроки</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0 w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                    ...
                    <div>
                      <p className="font-medium text-blue-900">Начало производства</p>
                      <p className="text-sm text-blue-700">Заказ перейдет в статус "В производство"</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0 w-6 h-6 bg-blue-100 rounded-full flex items-center justify-center">
                    ...
                    <div>
                      <p className="font-medium text-blue-900">Уведомления о прогрессе</p>
                      <p className="text-sm text-blue-700">Вы будете получать обновления о статусе заказа</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium">Анна Петрова</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Телефон:</span>
                    <span className="ml-2 font-medium">+7 (495) 123-45-67</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Email:</span>
                    <span className="ml-2 font-medium">orders@sofany.ru</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Время работы:</span>
                    <span className="ml-2 font-medium">9:00 - 18:00 (Пн-Пт)</span>
                  </div>
                </div>
              </div>
        ...
        {/* Навигация - скрыта на финальной странице */}
        {currentStep !== 6 && (
          <div className="flex justify-between mt-8">
            ...
            <div className="flex gap-4">
              {currentStep === 5 ? (
                ...
                >
                  {createOrderMutation.isLoading || uploadingDrawings ? (
