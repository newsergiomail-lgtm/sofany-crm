...
import SimpleQRCode from '../../components/Production/SimpleQRCode';
...
const CreateOrderWizard = () => {
  ...
  const createOrderMutation = useMutation(ordersAPI.create, {
    onSuccess: async (data) => {
      ...
      const newNumber = data?.order?.order_number || data?.order_number || `ЗАК-${newId}`;
      if (newId) {
            ...
            <div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-blue-50 rounded-lg p-4">
                  <h3 className="font-medium text-gray-900 mb-3">QR-код заказа</h3>
                  <div className="text-center">
                    <p className="text-sm text-gray-600 mb-4">
                      QR-код для быстрого доступа к заказу
                    </p>
                    <SimpleQRCode 
                      orderId={createdOrderId || 'Будет присвоен автоматически'}
                      orderNumber={orderNumber || `ЗАК-${Date.now()}`}
                      size={150}
                    />
                  </div>
                </div>
