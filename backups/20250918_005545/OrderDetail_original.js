...
import FinancialAnalytics from '../../components/Orders/FinancialAnalytics';
        ...
        <div className="mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate(`/orders/${id}/work-order`)}
                  className="p-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all duration-200"
                  title="Заказ-наряд (полный)"
                >
                  <Factory className="h-5 w-5" />
                </button>
                <button
                  onClick={() => navigate(`/work-order/${id}`)}
                  className="p-2 text-teal-600 hover:text-teal-900 hover:bg-teal-100 rounded-lg transition-all duration-200"
                  title="Заказ-наряд (простой)"
                >
                  <Factory className="h-5 w-5" />
                </button>
                <button
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.618fr_1fr] gap-6 lg:gap-7">
          ...
          <div className="space-y-6 mt-8 mb-8">
            ...
            <FinancialAnalytics 
              costValue={costValue}
              salePrice={price}
              onMarkupChange={(newSalePrice, newMarkupPercentage) => {
                setMarkupPercentage(newMarkupPercentage);
                // Здесь можно добавить логику обновления цены продажи
              }}
            />
