...
router.post('/generate-qr/:orderId', 
  authenticateToken, 
  checkPermission('can_generate_qr'),
  async (req, res) => {
  ...
  try {
    ...
    const order = orderResult.rows[0];
      ...
      // Деактивируем старые QR-коды для этого заказа
      await client.query(`
      ...
      const qrData = {
        ...
        signature: crypto.createHmac('sha256', process.env.QR_SECRET || 'default_secret')
          .update(`${orderId}-${stageId}-${Date.now()}`)
          .digest('hex')
      };
...
router.post('/mark-ready', 
  ...
  async (req, res) => {
    ...
    await client.query(`
      ...
      await client.query(`
        INSERT INTO order_transfers (order_id, to_stage_id, scanned_by, status, notes, transfer_type)
        VALUES ($1, $2, $3, 'ready', $4, 'normal')
      `, [orderId, null, userId, notes]);
