...
const orderSchema = Joi.object({
  ...
  additional_contact: Joi.string().allow('', null),
  preferred_contact: Joi.string().allow('', null),
...
router.get('/kanban', authenticateToken, async (req, res) => {
  try {
    ...
    // Получаем заказы со статусом 'in_production' и 'new' и их текущий этап производства
    const result = await client.query(`
      SELECT 
        o.id,
        o.order_number,
        ...
        o.has_elevator,
        o.floor,
        o.additional_contact,
        o.preferred_contact,
        o.blocked_by_materials,
        o.blocked_stages,
      ...
      WHERE o.status IN ('in_production', 'new')
...
router.post('/', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const { 
      ...
      additional_contact, preferred_contact,
    ...
    `, [
      ...
      additional_contact || null, preferred_contact || null,
...
router.put('/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(`
      ...
      short_description = COALESCE($12, orders.short_description),
      detailed_description = COALESCE($13, orders.detailed_description),
      calculator_data = COALESCE($14, orders.calculator_data),
      additional_contact = COALESCE($15, orders.additional_contact),
      preferred_contact = COALESCE($16, orders.preferred_contact),
